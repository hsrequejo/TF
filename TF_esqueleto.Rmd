...
Nas duas redes reais estudadas, a distribuição dos valores de G$_{norm}$ está como esperado, poucas espécies acima da média da rede e muitas espécies próximas da média ou abaixo dela. Isso mostra que apenas poucas espécies atingem valores altos de G$_{norm}$, sendo essa uma característica não comum entre nós da rede, assim como altos valores de centralidade

A variação de resolução $\gamma$ não parece influenciar G de forma tão intensa quanto a variação no acoplamento $\omega$, mas ela existe e com certeza é mais robusto selecionar nós com alto G$_{norm}$ como candidatos a centrais considerando a variação em $\gamma$ também, pois assim minimizamos a probabilidade de estarmos observando uma exceção dentro de um grande universo de parâmetros de resolução além de obter as espécies-chave de uma forma mais generalizada.

Caso exista uma variação significativa de G$_{norm}$ em relação a $\gamma$, podemos concluir que as espécies-chave detectadas por G$_{norm}$ possuem centralidade relativa a resolução, ou seja, espécies podem ter sua centralidade aumentada ou diminuída caso estejamos interessados em grupos maiores ou menores. \textbf[Realizar testes com redes sintéticas para verificar se isso ocorre muito]
