...
Os nós que possuem maiores valores de G$_{norm}$ parecem ser nós que posuem uma importância diferenciada dentro da rede, pois, ao permanecerem em diferentes grupos em diferentes camadas, mesmo após o aumento do acoplamento, mostra que os nós posuem uma relação mais igualitária entre diferentes grupos em camadas diferentes, o que os tornam nós com potencial de bons conectores entre diferentes grupos em camadas diferentes da rede.
...
A similaridade binária é bastante trivial e seu indíce é calculado da seguinte forma:

\begin{equation} \label{eq:sim_bin}
Sim_{bin} = \frac{\sum{j}^N\sum_{i}^N\delta(c_j, g_i)}{N}
\end{equation}
...
Além da comparação binária, podemos avaliar a similaridade levando em conta ranking do nó dentro do conjunto de corte escolhido. Para isso basta adicionarmos um fator de peso que é proporcional ao inverso da distância entre os nós dentro da equação \ref{eq:sim_bin}.

\begin{equation} \label{eq:sim_dist}
Sim_{dist} = \frac{\sum{j}^{N}\sum_{i}^{N}\frac{1}{1+|i-j|}\delta(c_j, g_i)}{N}
\end{equation}
...
\begin{table}[!h]
\caption{Nós ordenados de forma decrescente de acordo com seus valores de centralidade e $G_{norm}$ para e rede de Morcegos-plantas.}\label{table:centr_raw_bats}
\footnotesize
\begin{tabular}{cccccccccc}
\multicolumn{10}{l}{ \textbf{Closeness} } \\
\hline
Artjam & Carper & Cecobt & Cecpel & Artlit & Mactin & Manind & Manzap & Piparb & Carpap \\
...
\hline
Artjam & Carper & Artlit & Glosor & Stulil & Lepcur & Ceipen & Anocau & Syzjam & Stulud \\
0.3035 & 0.2642 & 0.1523 & 0.1441 & 0.1334 & 0.0803 & 0.0700 & 0.0697 & 0.0555 & 0.0552 \\
\hline
& & & & & & & & & \\
\multicolumn{10}{l}{ \textbf{Eigen Vector} } \\
\hline
Artjam & Carper & Artlit & Stulil & Cecpel & Glosor & Carcas & Cecobt & Manzap & Ficins \\
1.0000 & 0.8626 & 0.8017 & 0.3961 & 0.3685 & 0.3119 & 0.2744 & 0.2690 & 0.2664 & 0.2603 \\
\hline
& & & & & & & & & \\
\multicolumn{10}{l}{ \textbf{Degree} } \\
\hline
Artjam & Carper & Artlit & Stulil & Glosor & Carcas & Lepcur & Phyhas & Anocau & Carbre \\
149 & 138 & 112 & 72 & 65 & 44 & 43 & 40 & 30 & 29 \\
\hline
& & & & & & & & & \\
\multicolumn{10}{l}{ \textbf{Gnorm} } \\
\hline
Artjam & Syzjam & Artlit & Urobil & Stulil & Ingver & Carcas & Cecpel & Carbre & Artcon \\
...
\begin{table}[!h]
\caption{Similaridade binária entre $G_{norm}$ e outras métricas de centralidade.}\label{table:centr_sim_bin_bats}
\centering
\begin{tabular}{cccc}
\hline
...
\begin{table}
\caption{Similaridade ponderada pela distância dos rankings entre $G_{norm}$ e outras métricas de centralidade.}\label{table:centr_sim_dist_bats}
\centering
\begin{tabular}{cccc}
\hline
Closeness & Betweeness & Eigen Vector & Degree \\
...
Nas duas redes reais estudadas, a distribuição dos valores de G$_{norm}$ está como esperado, poucas espécies acima da média da rede e muitas espécies próximas da média ou abaixo dela. Isso mostra que apenas poucas espécies atingem valores altos de G$_{norm}$, sendo essa uma caractrística não comun entre nós da rede, assim como altos valores de centralidade

A variação de resolução $\gamma$ não parece influenciar G de forma tão intensa quanto a variação no acoplamento $\omega$, mas ela existe e com certeza é mais robusto selecionar nós com alto G$_{norm}$ como candidatos a centrais considerando a variação em $\gamma$ também, pois assim minizamos a probabilidade de estarmos observando uma exceção dentro de um grande universo de parâmetros de resolução além de obter as espécies-chave de uma forma mais generalizada.

Caso exista uma variação significativa de G$_{norm}$ em relação a $\gamma$, podemos concluir que as espécies-chave detectadas por G$_{norm}$  possuem centralidade relativa a resolução, ou seja, espécies podem ter sua centralidade aumentada ou diminuida caso estejamos interessados em grupos maiores ou menores. \textbf[Realizar testes com redes sintéticas para verificar se isso ocorre muito]

As espécies com maior G$_{norm}$ selecionadas possuem boa intersecção com as espécies centrais descritas em @Mello2019 e @Costa2016. Isto é interessante, já que o ranqueamento das espécies foi feito levando em conta a resistência de cada espécie ao parâmetro de acoplamento entre camadas $\omega$, algo muito diferente dos métodos de centralidade tradicionais. No entanto, ainda preciso comparar numericamente valores de centralidade usando a rede agregada binária e com peso de aresta igual a quantidade de interações em cada camada.
